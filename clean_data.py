...
class Dataset:
    ...
    def make_dataset(self) -> pd.DataFrame:
        ...
        for y in range(2013, 2026):
            for m in range(1, 13):
                ...
                try:
                    df_y_m = pd.read_csv(f"data/all_burglaries/{y!s}-{m}-metropolitan-street-burglary.csv")
    ...
    def clean_dataset(self) -> pd.DataFrame:
        ...
        lsoa_codes = dict(zip(df_lsoa_2011_to_lsoa_2021["LSOA11CD"], df_lsoa_2011_to_lsoa_2021["LSOA21CD"], strict=False))
        lsoa_names = dict(zip(df_lsoa_2011_to_lsoa_2021["LSOA11NM"], df_lsoa_2011_to_lsoa_2021["LSOA21NM"], strict=False))

        df_ward = pd.read_csv("lsoa_ward_data/LSOA_2021_to_Electoral_Ward_2024.csv")
        ward_codes = dict(zip(df_ward["LSOA21CD"], df_ward["WD24CD"], strict=False))
        ward_names = dict(zip(df_ward["LSOA21NM"], df_ward["WD24NM"], strict=False))
        lad_codes = dict(zip(df_ward["WD24CD"], df_ward["LAD24CD"], strict=False))
